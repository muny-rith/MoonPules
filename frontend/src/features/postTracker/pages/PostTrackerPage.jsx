...
import { Search, Filter, Calendar, ExternalLink, RefreshCw, BarChart2, DollarSign, Image as ImageIcon, Heart, MessageCircle, Share2, Edit2, Trash2, ChevronLeft, ChevronRight, Users, ChevronDown } from 'lucide-react';
...
export const PostTrackerPage = () => {
  ...
  const summary = useMemo(() => {
    if (!posts) return { total: 0, views: 0, reach: 0 };
    return {
      total: posts.length,
      views: posts.reduce((sum, p) => sum + (p.views_count || 0), 0),
      reach: posts.reduce((sum, p) => sum + (p.reach_count || 0), 0),
    };
  }, [posts]);
  ...
  return (
    <div>
      <div className="tasks-page-header" style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ margin: '0 0 8px 0', fontSize: '24px' }}>Moon Pulse Tracker</h1>
          <div style={{ display: 'flex', gap: '16px', color: 'var(--text-muted)', fontSize: '13px' }}>
            <span><strong>{summary.total}</strong> Total Posts</span>
            <span><strong>{summary.views.toLocaleString()}</strong> Total Views</span>
            <span><strong>{summary.reach.toLocaleString()}</strong> Total Reach</span>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '12px' }}>
          <button className="btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '8px' }} onClick={reload}>
      ...
      <div className="table-card" style={{ 
        ...
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
